...
class LegalPersonController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($corporate_name && $cnpj) {
            $query = $this->legalPerson->where('corporate_name', 'like', "%$corporate_name%")->where('cnpj', 'like', "%$cnpj%");
        } elseif ($corporate_name) {
            $query = $this->legalPerson->where('corporate_name', 'like', "%$corporate_name%");
        } elseif ($cnpj) {
            $query = $this->legalPerson->where('cnpj', 'like', "%$cnpj%");
        } else {
            $query = $this->legalPerson;
        }

        $legalPerson = $query->paginate(10)->appends($request->input());

        if($request->ajax()){
            $view = view('people.legalPerson.paginate_legalPerson.legalPerson_data', compact('legalPerson'))->render();
            $pagination = view('people.legalPerson.paginate_legalPerson.paginate', compact('legalPerson'))->render();
