...
@section('content')
    <div class="container">
        <div class="col-12">
            <h1 class="my-4 text-secondary text-center">Tipo de Pagamento</h1>
        </div>
        @if (session('status'))
        ...
        <div class="d-grid gap-2 d-md-flex justify-content-md-start my-4">
            <a href="{{route('typePayment.create')}}" class="btn btn-lg btn-success"> + Novo</a>
        </div>
        <div class="col-12 table-responsive">
            <table class="table align-middle">
                ...
                <tbody>
                    ...
                    <tr id="type-payment-{{$type->id}}">
                            <td>{{$type->id}}</td>
                            <td>{{$type->value}}</td>
                            <td>{{$type->description}}</td>
                            <td class="d-flex">
                                <a class="mr-3 btn btn-sm btn-outline-success" href="{{route('typePayment.edit', ['typePayment'=> $type->id])}}">Editar</a>
    ...
    <script>
        ...
        $(document).delegate('#btn-destroy', 'click', function(){
            ...
            $.ajax({
                url: "/public/domain/typePayment/"+id_type,
                type: 'DELETE',
                headers: {
