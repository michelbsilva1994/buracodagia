...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            ...
            <div class="col-8">
                <h2 class="text-secondary mt-2"> Tipo de Cobrança</h2>
            </div>
            <div class="col-8">
                <form action="{{ route('typeCharge.update', ['typeCharge'=>$type->id]) }}" method="post" class="mt-4 row" autocomplete="off">
                    @csrf
                    @method('PUT')
                    <div class="col-2">
                    ...
                    <div class="col-12">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="status" value="A" @if ($type->status == 'A') checked @endif>
                            <label class="form-check-label" for="status">Ativo</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="status" value="I" @if ($type->status == 'I') checked @endif>
                            <label class="form-check-label" for="status">Inativo</label>
                        </div>
                    </div>
                    <div class="mt-2">
                        <button type="submit" class="btn btn-block btn-success">Salvar</button>
                        <a href="{{route('typeCharge.index')}}" class="btn btn-danger">Cancelar</a>
                    </div>
                </form>
            </div>
        </div>
